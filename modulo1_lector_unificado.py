...
class VentanaDesconexion:
    ...
    def mostrar(self):
        ...
        centrar_ventana(self.ventana, 320, 200, margen_superior=200)
    ...
    def cerrar(self):
        ...
        Calcula el tiempo total de desconexión y lo guarda en BD.
        """
        if self.inicio_desconexion:
            delta = datetime.now() - self.inicio_desconexion
            ...
            if not self.motivo_guardado:
                # Si no escogió motivo → se guarda como "sin motivo"
                guardar_evento_desconexion("Desconexión sin motivo", tiempo_desconexion=segundos)
            else:
                # Si ya había un motivo → guardamos con tiempo total
                guardar_evento_desconexion(self.motivo_guardado, tiempo_desconexion=segundos)
