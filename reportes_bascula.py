...
class ReportesBasculaApp:
    def __init__(self, root):
        ...
        ttk.Label(filtro_frame, text="Fecha inicial (YYYY-MM-DD):").grid(row=0, column=2, padx=5)
        self.fecha_inicio = ttk.Entry(filtro_frame)
        self.fecha_inicio.grid(row=0, column=3, padx=5)

        # Filtro: Fecha final
        ttk.Label(filtro_frame, text="Fecha final (YYYY-MM-DD):").grid(row=0, column=4, padx=5)
    ...
    def consultar(self):
        ...
        try:
            ...
            fecha_fin = datetime.strptime(ff, "%Y-%m-%d")
        except ValueError:
